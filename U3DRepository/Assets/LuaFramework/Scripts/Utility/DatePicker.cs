...
public class DatePicker : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private ChoiceTime _choiceTime;
    ...
    public void Init()
    {
        _choiceTime = transform.GetComponentInParent<ChoiceTime>();
    ...
    {
        //判断拖拽的长度是否大于目标值
        if (Mathf.Abs(eventData.position.y - oldDragPos.y) >= _updateLength/_choiceTime._dragRatio)
        {
            int num;
            //判断加减
            ...
            {
                case DateType._year:
                    _data = _choiceTime._selectDate.AddYears(num);
                    break;
                case DateType._month:
                    _data = _choiceTime._selectDate.AddMonths(num);
                    break;
                case DateType._day:
                    _data = _choiceTime._selectDate.AddDays(num);
                    Debug.Log(_data.Day);
                    break;
                case DateType._hour:
                    _data = _choiceTime._selectDate.AddHours(num);
                    break;
                case DateType._minute:
                    _data = _choiceTime._selectDate.AddMinutes(num);
                    break;
                case DateType._second:
                    _data = _choiceTime._selectDate.AddSeconds(num);
                    break;
            }
            //判断是否属于时间段
            if (IsInDate(_data, _choiceTime._minDate, _choiceTime._maxDate))
            {
                _choiceTime._selectDate = _data;
                oldDragPos = eventData.position;
                _onDateUpdate();
            }
        }
    ...
    public void RefreshDateList()
    {
        ...
        for (int i = 0; i < _itemNum; i++)
        {
            ...
            {
                case DateType._year:
                    _data = _choiceTime._selectDate.AddYears(real_i);
                    break;
                case DateType._month:
                    _data = _choiceTime._selectDate.AddMonths(real_i);
                    break;
                case DateType._day:
                    _data = _choiceTime._selectDate.AddDays(real_i);
                    break;
                case DateType._hour:
                    _data = _choiceTime._selectDate.AddHours(real_i);
                    break;
                case DateType._minute:
                    _data = _choiceTime._selectDate.AddMinutes(real_i);
                    break;
                case DateType._second:
                    _data = _choiceTime._selectDate.AddSeconds(real_i);
                    break;
            }
            string str = "";
            if (IsInDate(_data, _choiceTime._minDate, _choiceTime._maxDate))
            {
                switch (_dateType)
                {
                    ...
                    case DateType._month:
                        ...
                        //str = _choiceTime.GetMothStr(_data.Month);
                        break;
                    case DateType._day:
