...
namespace LuaFramework {
    ...
    public class ResourceManager : Manager {
		...
		private List<string> ResourceNameList = new List<string>();
        ...
        Dictionary<string, AssetBundleCreateRequest> m_LoadScene = new Dictionary<string, AssetBundleCreateRequest>();
        ...
        public void Initialize(string manifestName, Action initOK) {
            ...
            //Debug.LogError(m_BaseDownloadingURL + " " + manifestName);
            LoadAsset<AssetBundleManifest>(manifestName, new string[] { "AssetBundleManifest" }, delegate(UObject[] objs) {
                if (objs.Length > 0) {
                    m_AssetBundleManifest = objs[0] as AssetBundleManifest;
                    //Debug.LogError("m_AssetBundleManifest:" + m_AssetBundleManifest);
                    m_AllManifest = m_AssetBundleManifest.GetAllAssetBundles();
                }
                if (initOK != null) initOK();
            });
        }
        public void LoadScenePrefab(string abName)
        {
            if (m_LoadScene.ContainsKey(abName)) {
                string streamPath = Application.streamingAssetsPath;
                AssetBundleCreateRequest assetbundle = null;
                Debug.LogError("LoadScenePrefab:"+streamPath + "/" + abName);
                assetbundle = AssetBundle.LoadFromFileAsync(streamPath + "/" + abName);
                m_LoadScene.Add(abName, assetbundle);
            }
        }
        ...
        public void LoadPrefab(string abName, string[] assetNames, LuaFunction func) {
            LoadAsset<GameObject>(abName.ToLower() + AppConst.ExtName, assetNames, null, func);
        }

        public void LoadBgImage(string name, GameObject obj)
        {
            StartCoroutine(this.GetAndSetLocalBgTexture(name, obj));
        }

        IEnumerator GetAndSetLocalBgTexture(string name, GameObject obj)
        {
            WWW www = new WWW("file://" + Util.DataPath + "/DownLoadImage/" + name);
            yield return www;
        ...
        string GetRealAssetPath(string abName) {
            ...
            //    return abName;
            //}
            //Debug.LogError(Util.DataPath + abName);
            if(File.Exists(Util.DataPath+ abName))
            {
                //Debug.LogError(Util.DataPath + abName);
                //Debug.LogError("GetRealAssetPath--------------------------------------------------------------------------------" + abName);
                return abName;
            }
            ...
            //Debug.LogError("GetRealAssetPath--------------------------------------------------------------------------------" + m_AllManifest.Length);
        ...
        IEnumerator OnLoadAsset<T>(string abName) where T : UObject {
            //Debug.LogError("OnLoadAsset1--->>>" + abName);
            ...
            if (bundleInfo == null) {
                ...
                if (bundleInfo == null) {
                    ...
                    //Debug.LogError("OnLoadAsset2--->>>" + abName);
                    yield break;
                }
            }
            ...
            for (int i = 0; i < list.Count; i++) {
                ...
                for (int j = 0; j < assetNames.Length; j++) {
                    ...
                    //Debug.LogError(assetPath + "--" + list[i].assetType);
        ...
        IEnumerator OnLoadAssetBundle(string abName, Type type)
        {
            ...
            {
                //Debug.LogError("加载地址:" + url+ ";abName:"+ abName);
                ...
                //Debug.LogError("DataPath::" + Util.DataPath);
                string md5 = Util.md5file(Util.DataPath + abName);
                //Debug.LogError("DataPath::" + Util.DataPath + abName);
                //Debug.LogError("url::" + url);
                //Debug.LogError("md5::"+md5);
                download = WWW.LoadFromCacheOrDownload(url, Hash128.Parse(md5), 0);
                //Debug.LogError("assetBundle::" + download.assetBundle.name);
